...
- **The terminal is GPU-rendered, with a new Terminal settings page** — The terminal now runs on xterm's WebGL2 renderer instead of the DOM renderer, so heavy output (`seq 1 200000` measured: 33 ms to drain, no frame over 14 ms) stays smooth instead of stuttering, and the renderer falls back to DOM automatically if the graphics driver rejects WebGL2 or the browser reclaims the context. Three addons joined it: search, replay serialization, and Unicode 11 width tables (CJK/emoji columns line up with the real terminal). Switching tabs now restores their content through xterm's own serializer rather than the main process's 200 000-character string cut, which used to slice escape sequences in half and replay garbage — and the settings page gained a **Terminal** tab covering color theme (inherit the app light/dark, or pin Solarized Light/Dark, One Dark, Monokai — live, without rebuilding the session), font size and family, both picked from dropdowns — the font list is read from the fonts actually installed on the machine (family names parsed out of each font file's `name` table, so `HackNerdFont-BoldItalic.ttf` shows up as `Hack Nerd Font` and there is one entry per family; macOS private `.`-prefixed families such as `.LastResort` are filtered out), with “follow the code font” / “follow the UI font size” as the defaults and a search box for the 300-odd entries, vertical padding, scrollback lines (applies to newly opened terminals), cursor shape/blink, copy-on-select, close confirmation (never / only when a foreground process is running / always), and a startup command run in every new terminal. The dock's own theme menu writes back into the same setting, so the two can no longer disagree.
