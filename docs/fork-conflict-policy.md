...
1. **绝不静默吞改动**。`AGENTS.md` 明令禁止 `-X ours` / `-X theirs`；本策略用「显式取边 +
   补丁重放」替代无脑选边，且任何策略未覆盖的冲突一律**停下并开 issue**，不推送。
2. **上游修复优先**。同一处 bug 上游也修了 → 取上游（你的原话：「修的是同一个 bug 就无所谓」）。
3. **生成物永不手工解**。`package-lock.json`、各 `*-manifest.json`、`pi-ai-catalog.json`、
   `announcements.json` 冲突时取任一边后**重新生成**（见第 3 节）。
4. **fork 身份文件以 fork 为准**（见第 2 节 A 区）。
...
其余全部文件（占绝大多数）。上游是主体，取上游即正确。

**`package-lock.json` 属于 D 区（取上游）。** fork 没有新增依赖，lock 由上游拥有；
取上游可保证依赖树与上游 `package.json` 一致。（早期草稿把它归入生成物，会保留
fork 的旧 lock 状态而与上游 deps 不一致，已纠正。）
